...
{
    public partial class F_Shingeki : Form
    {
        ...
        private void Inicio()
        {
            foreach (PictureBox item in Controls.OfType<PictureBox>())
            {
                ...
                item.Image = Properties.Resources.FundoCartaShingeki; // poe as imagens para aparecer apenas o fundo;
                item.Enabled = true;
            }
            Posicoes();
        }
        ...
        private void Desvirar(bool Check)
        {
            ...
            foreach (PictureBox item in Controls.OfType<PictureBox>())
            {
                if (int.Parse(String.Format("{0}", item.Tag)) == tags[0] || int.Parse(String.Format("{0}", item.Tag)) == tags[1])
                {
                    ...
                    {
                        item.Image = Properties.Resources.FundoCartaShingeki;
                        item.Refresh();
                    }
                }
            }
